...
app = FastAPI(title="Gym Planner API")


def calculate_nutrition(weight: float, goal: str):
    if goal == "reduce":
...
@app.get("/gym-plan")
def get_gym_plan(body_weight_kg: float, goal: str):
    nutrition = calculate_nutrition(body_weight_kg, goal)
...
@app.get("/")
def root():
    return {"message": "Gym Planner API is running. Use /gym-plan?body_weight_kg=70&goal=bulk"}


if __name__ == "__main__":
    response = requests.get("http://127.0.0.1:8000/gym-plan", params={
        "body_weight_kg": 75,
        "goal": "bulk"
    })
    print(response.json())